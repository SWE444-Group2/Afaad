  ...
  useEffect(() => {
    ...
    dataref.on("value", (snapshot) => {
      ...
      for (let productID in productsList) {
        if (
          userType === "Investor" &&
          productsList[productID].status == "Accepted" &&
          productsList[productID].InvestorsList != null
        ) {
          ...
          for (let i in inv) {
            if (inv[i] == userID) {
              ...
              if (
                productsList[productID].InvestorsList[userID].status !=
                "Pending"
              )
                counter++;
            }
          ...
          userType === "Entrepreneur" &&
          productsList[productID].userID == userID &&
          productsList[productID].status != "Pending"
        ) {
          PendingProductList.push(productsList[productID]);
          counter++;
        }
                      ...
                      >
                        <View style={styles.item}>
                          <Text
                            style={[
                              Titlestyles.subTitle,
                              Titlestyles.DescText,
                              { fontSize: 16, width: "75%" },
                            ]}
                          >
                            يوجد لديك طلب استثمار جديد في
                            <Text
                              style={{
                                color: "#247ba0",
                  ...
                  item.InvestorsList[userID].status != "Pending" && (
                    <TouchableOpacity>
                      ...
                      {item.InvestorsList[userID].status == "Rejected" && (
                        <TouchableOpacity onPress={() => getInfo(item.productID,userID)}>
                          <View style={styles.item}>
                            <Text
                              style={[
                                Titlestyles.subTitle,
